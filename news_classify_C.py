#To classify the txt score in Company category
...
#在做这一步前，需要将company的几种categories的文件夹放在corpora中所建的company文件夹里
company = LazyCorpusLoader('Company', CategorizedPlaintextCorpusReader, r'(?!\.).*\.txt', cat_pattern=r'(neg1|zero|pos1|pos2)/.*', encoding='utf-8') 
#在corpus中导入Company文件，文件是txt格式，分为四个categories，分别是neg1,zero,pos1和pos2
...
documents = [(list(company.words(fileid)),category)
              for category in company.categories()
			  for fileid in company.fileids(category)] #document是以list的方式储存每个news，并在后面添上category
...
training_set = featuresets[:103]
testing_set = featuresets[103:]

classifier = nltk.NaiveBayesClassifier.train(training_set)

classifier_f = open("naivebayes_C.pickle","rb")
...
# save_classifier = open("naivebayes_C.pickle","wb")
